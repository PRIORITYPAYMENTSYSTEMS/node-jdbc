...
const keepAliveFunction = async function(connection, query) {
    try {
        const stmt = await connection.createStatement();
        await stmt.execute(query);
        console.log("%s - Keep-Alive", new Date().toUTCString());
    }
    catch (err) {
...
const addConnection = async function(url, props, ka, maxIdle) {
    const connection = new Connection(await dm.getConnection(url, props));

    const connObj = {
        uuid: uuid.v4(),
        conn: connection,
        keepAlive: ka.enabled ? setInterval(keepAliveFunction, ka.interval, connection, ka.query) : false
    };
...
class Pool {
    ...
    async release(connection) {
        ...
        if (!isNil(connection.lastIdle)) connection.lastIdle = new Date().getTime();
