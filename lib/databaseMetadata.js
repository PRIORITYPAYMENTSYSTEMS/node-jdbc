...
class DatabaseMetadata {
    ...
    getTables(catalog, schemaPattern, tableNamePattern, types = []) {
        return promiseForMethod(() => {
            if (!(
                ...
                (isArray(types) && (isEmpty(types) || types.every(isString)))
            )) {
                throw new Error("INVALID ARGUMENTS");
            }
    ...
    allTablesAreSelectable() {
        return promiseForMethod(() => this.dbm.allTablesAreSelectableSync());
    }
    ...
    getUDTs(catalog, schemaPattern, typeNamePattern, types = []) {
        return promiseForMethod(() => {
            if (!(
                ...
                (isNil(schemaPattern) || isString(schemaPattern)) &&
                (isNil(typeNamePattern) || isString(typeNamePattern)) &&
                (isArray(types) && (isEmpty(types) || types.every(isInteger)))
            )) {
                throw new Error('INVALID ARGUMENTS');
            }
    ...
    supportsResultSetConcurrency(type, concurrency) {
        return promiseForMethod(() => {
            ...
            return this.dbm.supportsResultSetConcurrency(type, concurrency);
        });
    }
...
jinst.events.once('initialized', DatabaseMetadata.onInitialized);
